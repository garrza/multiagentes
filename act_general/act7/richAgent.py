...
class RichAgent(ap.Agent):
    """
    Agente que utiliza mensajes para comunicarse con otros agentes.
    """

    def setup(self):
        self.wealth = np.random.randint(10, 20)  # Riqueza inicial
    ...
    def step(self):
        ...
        if self.wealth > 1 and self.partner:  # Regla simple para transferir riqueza
            self.send_msg(self.partner.id, "transfer", {"coins": 2})
            self.wealth -= 1
